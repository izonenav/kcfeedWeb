...
class LIB {
	...
	static function PLog($log, $path="")
	{
		...
		fwrite($log_file, date("Y-m-d H:i:s"). " ". $php_filename. " ==> ". $log. PHP_EOL);  
		fclose($log_file);  
	}
	static function PLog2($log, $path="")
	{
		if(is_array($log))
		{
			$logs = "Array { \r\n";
		...
		$file_path = $_SERVER['DOCUMENT_ROOT']. "/log2.txt";
		$php_filename = $_SERVER['PHP_SELF'];
