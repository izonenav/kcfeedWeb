...
class Gallery
{
	...
	function getCardInfo($limitStartRow,$limitFetchSize)
	{
		$this->db->que = " select * from board LIMIT  ". $limitStartRow. ", ". $limitFetchSize;
		$this->db->query();	
		$rows = $this->db->getRows();
		$cardInfo = $this->getCardInfoList($rows);
		return $cardInfo;
	}

	function getTotalCount()
	{
		$this->db->que = " SELECT COUNT(seq) FROM board ";
		$this->db->query();
		$totalCount = $this->db->getOne();
		return $totalCount;
	}
	
	function getCardInfoList($rows)
	{
		$count = count($rows);
		for($i=0;$i<count($rows);$i++)
		{
			...
			$cardInfo .= "
								  <div class='card'>
									<img class='card-img-top' src='".$row["imgUrl"]."' alt='Card image cap' width='200' height='40'>
